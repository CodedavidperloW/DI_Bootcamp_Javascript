...
const cards = document.querySelectorAll(".cards .card"); //The querySelectorAll() method returns all elements that matches a CSS selector(s). This is connecting JS to CSS elements .cards .card// Question, why is the container div ".cards" selected alongwith ".card", why is ".card" not selected by itself?
cards.forEach((card) => { ////The forEach() method executes a provided function once for each array element.
    card.addEventListener("click", () => {//.addEventListener is adding an 'event' to the element .card each time the event of clicking occurs on the card. 
        ...
        card.classList.add("clicked");// This enables a new class to be added to each card that is clicked.
